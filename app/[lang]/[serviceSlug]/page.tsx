...
export async function generateMetadata({ params }: ServicePageProps): Promise<Metadata> {
  ...
  const serviceId = getServiceBySlug(params.serviceSlug, lang);
  if (!serviceId) {
    return {};
  }
  
  // Generate metadata for the service
  return generateServiceMetadata(serviceId, lang);
}
...
export default function ServicePage({ params }: ServicePageProps) {
  ...
  return (
    <>
      ...
      <Hero
        lang={lang}
        title={service.names[lang]}
        subtitle={service.descriptions[lang]}
        ctaText={t('getStarted')}
        ctaLink={`/${lang}/${getServiceSlug(serviceId, lang)}/${getGameSlug('puzzle', lang)}`}
        secondaryCtaText={t('contactUs')}
        secondaryCtaLink={`/${lang}/contact`}
        imageSrc={heroImage}
        imageAlt={service.names[lang]}
      ...
      <ContentBlock
        title={isTherapeutic 
          ? (lang === 'en' ? 'Therapeutic Gaming for Mental Health' : 'Therapeutisches Spielen für psychische Gesundheit')
          : (lang === 'en' ? 'Community Support for Mental Wellbeing' : 'Community-Unterstützung für psychisches Wohlbefinden')}
        content={
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-neutral-50 dark:bg-neutral-800 p-6 rounded-lg">
              ...
              <h3 className="text-xl font-bold mb-2">
                {isTherapeutic
                  ? (lang === 'en' ? 'Evidence-Based Approach' : 'Evidenzbasierter Ansatz')
                  : (lang === 'en' ? 'Safe Environment' : 'Sichere Umgebung')}
              </h3>
              <p>
                {isTherapeutic
                  ? (lang === 'en'
            ...
            <div className="bg-neutral-50 dark:bg-neutral-800 p-6 rounded-lg">
              ...
              <h3 className="text-xl font-bold mb-2">
                {isTherapeutic
                  ? (lang === 'en' ? 'Accessible Anytime' : 'Jederzeit zugänglich')
                  : (lang === 'en' ? '24/7 Support' : '24/7 Unterstützung')}
              </h3>
              <p>
                {isTherapeutic
                  ? (lang === 'en'
            ...
            <div className="bg-neutral-50 dark:bg-neutral-800 p-6 rounded-lg">
              ...
              <h3 className="text-xl font-bold mb-2">
                {isTherapeutic
                  ? (lang === 'en' ? 'Engaging Experience' : 'Ansprechende Erfahrung')
                  : (lang === 'en' ? 'Shared Experiences' : 'Geteilte Erfahrungen')}
              </h3>
              <p>
                {isTherapeutic
                  ? (lang === 'en'
      ...
      <section className="section bg-neutral-50 dark:bg-neutral-800">
        <div className="container-custom">
          <SectionTitle
            title={lang === 'en' ? 'Explore Our Game Categories' : 'Entdecken Sie unsere Spielkategorien'}
            subtitle={lang === 'en'
              ? `Discover how our ${service.names[lang].toLowerCase()} can help with different aspects of mental wellbeing through various game types.`
              : `Entdecken Sie, wie unsere ${service.names[lang].toLowerCase()} durch verschiedene Spieltypen bei verschiedenen Aspekten des psychischen Wohlbefindens helfen können.`}
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {gameIds.map((gameId) => (
              <FeatureCard
                ...
                description={lang === 'en'
                  ? `Explore our ${service.names[lang].toLowerCase()} for ${getGameName(gameId, lang).toLowerCase()} and discover how they can support your mental wellbeing.`
                  : `Entdecken Sie unsere ${service.names[lang].toLowerCase()} für ${getGameName(gameId, lang).toLowerCase()} und erfahren Sie, wie sie Ihr psychisches Wohlbefinden unterstützen können.`}
                ...
                link={`/${lang}/${getServiceSlug(serviceId, lang)}/${getGameSlug(gameId, lang)}`}
      ...
      <section className="section bg-white dark:bg-neutral-900">
        <div className="container-custom text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            {lang === 'en'
              ? `Ready to experience our ${service.names[lang].toLowerCase()}?`
              : `Bereit, unsere ${service.names[lang].toLowerCase()} zu erleben?`}
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href={`/${lang}/${getServiceSlug(serviceId, lang)}/${getGameSlug('puzzle', lang)}`} className={isTherapeutic ? 'btn-primary' : 'btn-secondary'}>
              {t('getStarted')}
            </Link>
